...
def get_rval_data(X, y, lr_outputs, nn_outputs, latent_type, filename, coloring='k', title='Birthweight', save=False):
    ...
    for idx, i in enumerate(y):
        ...
        if save:
            ...
            plt.tight_layout()
            plt.savefig(DIR + 'figure/VAE/%s_with_race_%s.pdf' % (latent_type, targets[idx]))
            plt.show()
...
def perform_holdout_test(rvals, all_data, index, targets, data_type, coloring, title='Birthweight'):
    ...
    for idx1 in range(len(targets)):
        ...
        plt.tight_layout()
        plt.savefig(DIR + 'figure/VAE/holdout_test_with_race_%s_%s.pdf' % (data_type, targets[idx1]))
        plt.show()
...
def model_birthweight(baseline, directory, latent_type, color='k'):
    ...
    race = pd.DataFrame(X['child_race___2'])
    X = pd.read_csv(baseline, sep='\t')
    X = pd.concat([X, race], axis=1)
    X.columns = ['adv', 'psych', 'race']
    ...
    # LR
    filename = directory + 'LR/LR_without_race_outputs.csv'
    if os.path.exists(filename):
        lr_outputs = pd.read_csv(filename, sep='\t')
    else:
        lr_model = custom_build_model(input_size, output_size, width, depth, kw, okw, hidden=hidden)        
    ...
    # NN
    filename = directory + 'NN/NN_without_race_outputs.csv'
    if os.path.exists(filename):
        nn_outputs = pd.read_csv(filename, sep='\t')
    else:
        nn_model = custom_build_model(input_size, output_size, width, depth, kw, okw, hidden=hidden)
    ...
    # Rvals
    filename = directory + 'rvals/rvals_without_race.csv'
    if os.path.exists(filename):
        rvals_without_race = pd.read_csv(filename, sep='\t')
    else:
        rvals_without_race = get_rval_data(X, y, lr_outputs, nn_outputs, latent_type, filename, save=False)
    
    # Permuted
    onlyfiles = [f for f in listdir(directory) if isfile(join(directory, f)) and 'permuted' in f]
    all_data = []
    index = []
    all_data.append(rvals_without_race)
    index.append('Race')
    input_size = 3
    for i in onlyfiles:
        ...
        # LR
        filename = directory + 'LR/LR_with_race_' + i
        if os.path.exists(filename):
        ...
        # NN
        filename = directory + 'NN/NN_with_race_' + i
        if os.path.exists(filename):
        ...
        # Rvals
        filename = directory + 'rvals/rvals_with_race_' + i
        if os.path.exists(filename):
